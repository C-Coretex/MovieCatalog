...
namespace MovieCatalog.Application.Services.UnitTests.AppServiceTests
{
    public class MovieCatalogAppServiceTests
    {
        ...
        public async Task GetMoviesByTitle_ShouldReturnCorrectDataAndSaveHistory(bool existingTitle)
        {
            ...
            var result = await _movieCatalogAppService.GetMoviesByTitle(title).ToListAsync();
        ...
        public async Task GetMoviesByTitle_ShouldThrowAndLogOnRepositoryFailure()
        {
            ...
            var func = async () => await _movieCatalogAppService.GetMoviesByTitle("").ToListAsync();
        ...
        {
            var titles = new Faker().Random.WordsArray(moviesCount / 3 * 2);
            return new Faker<FullMovieOmdbDto>()
                .CustomInstantiator(f => new FullMovieOmdbDto(
                    ...
                    Released: f.Random.Bool(0.9f) ? DateOnly.FromDateTime(f.Date.Past(50, DateTime.Today)) : null,
                    Runtime: $"{f.Random.Int(80, 180)} min",
                    Genres: f.PickRandom(new[]
                    {
                    ...
                    ImdbRating: f.Random.Bool(0.9f) ? f.Random.Float(1.0f, 10.0f) : null,
                    ImdbVotes: f.Random.Bool(0.9f) ? f.Random.Int(100, 2000000) : null,
